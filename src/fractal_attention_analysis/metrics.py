...
class AttentionMetrics:
    ...
    def compute_interpretability_score(
        self, attention_weights: np.ndarray, fractal_dimension: float
    ) -> float:
        """
        ...
        uniformity = 1 - self.compute_uniformity(
            attention_weights
        )  # Invert (less uniform = more interpretable)
        ...
        score = 0.3 * sparsity + 0.3 * concentration + 0.2 * uniformity + 0.2 * fractal_score
    ...
    def compute_attention_flow(self, attention_weights: np.ndarray) -> Dict[str, float]:
        ...
        if total == 0:
            return {"forward_flow": 0.0, "backward_flow": 0.0, "self_attention": 0.0}
    ...
    def compute_all_metrics(
        self, attention_weights: np.ndarray, fractal_dimension: Optional[float] = None
    ) -> Dict[str, Any]:
        """
        ...
        if fractal_dimension is not None:
            ...
            metrics["interpretability_score"] = self.compute_interpretability_score(
                attention_weights, fractal_dimension
            )
