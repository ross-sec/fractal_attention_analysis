...
class FractalTransforms:
    ...
    def compute_self_similarity(
        ...
        resized = attention_matrix[:target_size*scale_factor:scale_factor,
                                   :target_size*scale_factor:scale_factor]
    ...
    def golden_ratio_scoring(self, attention_weights: np.ndarray) -> np.ndarray:
        ...
        golden_weights = np.array([
            self.phi ** (-i / seq_len) for i in range(seq_len)
        ])
