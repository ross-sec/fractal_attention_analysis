...
Examples:
  ...
  faa-compare --model1 gpt2 --model2 distilgpt2 --text "Test sentence"
    ...
    parser.add_argument(
        "--head", type=int, default=0, help="Attention head index to analyze (default: 0)"
    )

    # Output options
    parser.add_argument("--output", type=Path, help="Path to save analysis results (JSON format)")
    parser.add_argument("--save-viz", type=Path, help="Directory to save visualizations")
    parser.add_argument(
        "--format",
    ...
    parser.add_argument(
        "--max-gpu-memory", type=int, default=20, help="Maximum GPU memory in GB (default: 20)"
    )
...
def run_analysis(args):
    ...
    device_manager = DeviceManager(prefer_gpu=prefer_gpu, max_gpu_memory_gb=args.max_gpu_memory)

    # Initialize analyzer
    if not args.quiet:
        print(f"Initializing Fractal-Attention Analyzer for model: {args.model}")

    analyzer = FractalAttentionAnalyzer(model_name=args.model, device_manager=device_manager)
